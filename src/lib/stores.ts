	...
	return {
		...
		init: async () : Promise<void> => {
			...
			if (stored_theme !== null) {
				set(stored_theme);
				if (document.firstElementChild !== null) document.firstElementChild.setAttribute('color-scheme', stored_theme);
			}
			...
			// Fallback for first run
				...
				set(preferredTheme);
				if (document.firstElementChild !== null) document.firstElementChild.setAttribute('color-scheme', preferredTheme);
		...
		setDarkTheme: async () : Promise<void> => {
			set('dark');
			if (document.firstElementChild !== null) document.firstElementChild.setAttribute('color-scheme', 'dark');
			await asyncLocalStorage.setItem(STORAGE_KEY, 'dark');
		},

		setLightTheme: async () : Promise<void> => {
			set('light');
			if (document.firstElementChild !== null) document.firstElementChild.setAttribute('color-scheme', 'light');
